...
import { db } from './lib/firebase/config'; // Adjust the import based on your structure
...
app.use(cors({ origin: 'https://portraitify-1essj9f7c-oluwatomisin-badmus-projects.vercel.app/' })); // Replace with your frontend URL
...
app.post('/webhook', async (req, res) => {
    ...
    switch (event.type) {
        case 'checkout.session.completed':
            ...
            await handleCheckoutSession(session); // Call the function to handle the checkout session
            break;
...
const handleCheckoutSession = async (session) => {
    const email = session.customer_email; // Get the email from the session
